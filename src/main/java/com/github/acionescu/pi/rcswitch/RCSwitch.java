 ...
 * <p />
 * Just connect the DATA IN Pin with the pin provided in the constructor. The VCC with +5V (PIN2) and GND with Ground
 * (PIN6).
 * <p />
 * Usage example:
 * <p />
...
public class RCSwitch {
     ...
     *            Code of the switch group (refers to DIP switches 1..5 where "1" = on and "0" = off, if all DIP
     ...
     *
     * @param nAddressCode
     *            Number of the switch group (1..4)
     * @param nChannelCode
     *            Number of the switch itself (1..4)
     */
    ...
    /**
     * Returns a char[13], representing the Code Word to be send. A Code Word consists of 9 address bits, 3 data bits
     * and one sync bit but in our case only the first 8 address bits and the last 2 data bits were used. A Code Bit can
     * have 4 different states: "F" (floating), "0" (low), "1" (high), "S" (synchronous bit)
     *
     * +-------------------------------+--------------------------------+-----------------------------------------+----
     * -------------------------------------+----------------------+------------+ | 4 bits address (switch group) | 4
     * bits address (switch number) | 1 bit address (not used, so never mind) | 1 bit address (not used, so never mind)
     * | 2 data bits (on|off) | 1 sync bit | | 1=0FFF 2=F0FF 3=FF0F 4=FFF0 | 1=0FFF 2=F0FF 3=FF0F 4=FFF0 | F | F | on=FF
     * off=F0 | S |
     * +-------------------------------+--------------------------------+------------------------------------
     * -----+-----------------------------------------+----------------------+------------+
     *
     ...
     *            Number of the switch itself (1..4)
     * @param bStatus
     *            Wether to switch on (true) or off (false)
     *
     * @return char[13]
     */
    ...
    /**
     * Convenience method to control a remote switch (Type B with two rotary/sliding switches)
     * 
     * @param nAddressCode
     * @param nChannelCode
     * @param state
     ...
     */
    public void sendSwitchCommand(int nAddressCode, int nChannelCode, boolean state) {
	sendTriState(getCodeWordB(nAddressCode, nChannelCode, state));
    }

    /**
     * Convenience method to control a remote switch (Type A with 10 pole DIP switches)
     * 
     * @param switchGroupAddress
     * @param switchCode
     */
    public void sendSwitchCommand(BitSet switchGroupAddress, int switchCode, boolean state) {
	if (switchGroupAddress.length() > 5) {
	    throw new IllegalArgumentException("switch group address has more than 5 bits!");
	}
	this.sendTriState(this.getCodeWordA(switchGroupAddress, switchCode, state));
    }

    /**
     * @return the transmitterPinName
     */
    public String getTransmitterPinName() {
	return transmitterPinName;
    }

    /**
     * @param transmitterPinName
     *            the transmitterPinName to set
     */
    public void setTransmitterPinName(String transmitterPinName) {
	this.transmitterPinName = transmitterPinName;
    }
